...
try {
  ...
  await navigate('IDE'); await panel('Agent')
  await page.getByRole('button', { name: '请先配置模型', exact: true }).click()
  await expect(page.getByRole('button', { name: '添加供应商', exact: true })).toBeVisible()
  await expect(page.getByRole('navigation', { name: '主要导航' }).getByRole('button', { name: '供应商', exact: true })).toHaveAttribute('aria-current', 'page')
  ...
  await expect(page.getByRole('button', { name: '请先配置模型', exact: true })).toBeHidden()
